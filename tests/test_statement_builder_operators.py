...
class TestStatementBuilderOperators(unittest.TestCase):
    ...
    def test_and_when_functionality(self):
        ...
        if statement.conditions:
            ...
            self.assertEqual(statement.conditions[0].operator.value, Operator.GT.value)
            self.assertEqual(statement.conditions[0].right, 18)
            
            self.assertEqual(statement.conditions[1].left, "$.user.verified")
            self.assertEqual(statement.conditions[1].operator.value, Operator.EQ.value)
    ...
    def test_common_statements_premium_user(self):
        """Testa o método premium_user da classe CommonStatements."""
        from tests.common_statements import CommonStatements
        
        # Obter o statement para usuário premium
        statement = CommonStatements.premium_user("premium_state")
        
        # Verificar se as condições estão corretas
        self.assertIsNotNone(statement.conditions)
        if statement.conditions:  # Verifica se não é None
            self.assertEqual(len(statement.conditions), 2)
            
            # Verifica as condições específicas
            self.assertEqual(statement.conditions[0].left, "$.user.purchases")
            # Comparar valores do enum em vez dos enums
            self.assertEqual(statement.conditions[0].operator.value, Operator.GT.value)
            self.assertEqual(statement.conditions[0].right, 5)
            
            self.assertEqual(statement.conditions[1].left, "$.user.totalSpent")
            self.assertEqual(statement.conditions[1].operator.value, Operator.GT.value)
            self.assertEqual(statement.conditions[1].right, 100)
        
        # Verifica o estado de destino
        self.assertEqual(statement.next_state, "premium_state")
