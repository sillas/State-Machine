...
class StatementEvaluator:
    ...
    def __init__(self, statements: Optional[list[Statement]]):
        ...
        # Convert to the dictionary format expected by StatementEvaluator
    ...
    def _validate_statements(self, statements: list[dict[str, Any]]) -> None:
        ...
        if last_statement.get("sttm") is not None:
            raise ValueError(
                "Last statement must be a default case with sttm=None")
    ...
    def _get_value(self, value_expr: Any, data: dict[str, Any]) -> Any:
        ...
        try:
            ...
            raise ValueError(
                f"Error extracting value at path '{value_expr}': {str(e)}")
    ...
    def _apply_operator(self, left: Any, op: str, right: Any) -> bool:
        ...
        try:
            ...
            raise ValueError(
                f"Type mismatch for operation '{op}' between {type(left)} and {type(right)}")
    ...
    def _parse_condition(self, condition_str: str) -> dict[str, Any]:
        ...
        if len(parts) != 3:
            raise ValueError(
                f"Invalid condition format: {condition_str}. Expected 'left op right'")
        ...
        while i < len(self.statements):
            ...
            if statement.get("bool_ops") is not None:
                ...
                if i + 1 >= len(self.statements):
                    raise ValueError(
                        "Boolean operation requested but no next statement exists")
                ...
                if bool_op == BooleanOperator.AND.value:
                    ...
                    raise ValueError(
                        f"Unsupported boolean operator: {bool_op}")
